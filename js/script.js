document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadNovedades() {
    if (!novedadesSection || !novedadesContainer) return;
    try {
        ...
        card.innerHTML = `
          ...
          <div class="novedad-body">
            <div class="novedad-meta">
              ...
              <span class="novedad-date">${item.date || ""}</span>
            </div>
            <h3 class="novedad-title">${item.title}</h3>
            <p class="novedad-summary">${item.summary || ""}</p>
            ${item.content ? `
